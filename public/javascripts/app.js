...
class App extends React.Component {
  ...
  render() {
    ...
    for(var i=0; i<this.state.movies.length; i++){
      ...
      cardList.push(
        <Movie handleClickParent={this.handleClickParent} isLike={isLike} viewOnlyLike={this.state.viewOnlyLike} idMovieDB={this.state.movies[i].id} poster_path={this.state.movies[i].poster_path} title={this.state.movies[i].title} overview={this.state.movies[i].overview} />
      );
    }
